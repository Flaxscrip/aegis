...
# Registry the agents anchor on. `local` bars authoring the ephemeral docs credential-accept rides
# (ephemeralRegistry is hardcoded to hyperswarm; SANDBOX-PROFILE §6) → accept fails. `hyperswarm` on an
# internal:true node with NO mediator is a plain local DB write (confirmed, offline) that also satisfies
# the consistency check. Default stays local; run `REGISTRY=hyperswarm deploy/two-node/harness-hearthold-delivery.sh`.
...
gk_did(){ docker run --rm --network "$2" -v "$HH/$3:/data" -v "$HH/deploy/two-node/hh:/app/hh" \
  -e HEARTHOLD_NODE_URL="$4" -e HEARTHOLD_REGISTRY=$REGISTRY -e HEARTHOLD_DATA_ROOT=/data -e HEARTHOLD_PASSPHRASE=$PP \
...
docker run -d --name aegis-hh-subject --network aegis-peer \
  ...
  -e HEARTHOLD_REGISTRY=$REGISTRY -e HEARTHOLD_DATA_ROOT=/data -e HEARTHOLD_PASSPHRASE=$PP \
...
CRED_DID=$(docker run --rm --network archon_default -v "$HH/data-hh-issuer:/data" -v "$HH/deploy/two-node/hh:/app/hh" \
  -e HEARTHOLD_NODE_URL=http://drawbridge:4222 -e HEARTHOLD_REGISTRY=$REGISTRY -e HEARTHOLD_DATA_ROOT=/data -e HEARTHOLD_PASSPHRASE=$PP \
...
HELD=$(docker run --rm --network aegis-peer -v "$HH/data-hh-subject:/data" -v "$HH/deploy/two-node/hh:/app/hh" \
  -e HEARTHOLD_NODE_URL=http://aegis-nodeb-proxy:4299 -e HEARTHOLD_REGISTRY=$REGISTRY -e HEARTHOLD_DATA_ROOT=/data -e HEARTHOLD_PASSPHRASE=$PP \
